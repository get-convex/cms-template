...
export const zodOptionalString = (zs?: ZodString) =>
  z.optional(z.union([z.literal(""), zs || z.string()]));
...
export const postContentZod = {
  title: z.string(required).min(2).max(60),
  slug: zodSlug,
  summary: zodOptionalString(
    z.string().min(10, "10 characters min").max(200, "200 characters max"),
  ),
  content: z.string(),
...
export default defineSchema({
  ...
  versions: defineTable(zodToConvexFields(versionsZod))
    .index("by_postId", ["postId"])
    .index("by_slug", ["slug"]), // to lookup old slugs for redirects
  images: defineTable(zodToConvexFields(imagesZod)).index("by_storageID", [
    "storageId",
  ]),
});
