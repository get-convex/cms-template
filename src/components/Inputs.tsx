...
export function TextField<Schema extends FieldValues>({
  ...
  const ifRequired = required ? { required: true, "aria-required": true } : {};
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem
          className={itemClass || "grid grid-cols-4 gap-4 mb-4 items-center"}
        >
...
interface MarkdownFieldProps<Schema extends FieldValues>
  extends TextFieldProps<Schema> {
  rows?: number;
}
...
export function ImageField<Schema extends FieldValues>({
  form,
}: CommonProps<Schema>) {
  ...
  return (
    <div className="grid grid-cols-4 gap-x-4 items-center">
      <TextField
        name={"imageUrl" as FieldPath<Schema>}
        form={form}
        itemClass="space-y-2 grid grid-cols-subgrid col-span-4 sm:col-span-3 gap-4 mb-4 items-center"
        controlClass="col-span-3 sm:col-span-2 row-span-2"
      />
